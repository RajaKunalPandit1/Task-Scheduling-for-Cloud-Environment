...
We sincerely thank our cluster head, Prof. (Dr.) Neelu J. Ahuja, for her great support in doing our project at SoCS.
...
In paper [1],of “Optimization of tasks in cloud computing based on MAX-MIN, MIN-MIN and priority” it is found that considering the growing use of cloud computing and the need for optimal use of resources in the cloud, and attention to users that pay for services they use based on their pay-as-you-go basis. There should be a quicker way for users to decrease the user's waiting time and task’s waiting time. [2] Task scheduling is the main problem in cloud computing which reduces the system performance. To improve system performance, there is a need for an efficient task-scheduling algorithm. Existing task-scheduling algorithms focus on task-resource requirements, CPU memory, execution time and execution cost. However, these do not consider network bandwidth.
...
●	To schedule tasks on a real-time system. Achieving a high system throughput. Improving load balance 
Methodology
The project flow (Fig 1) starts from taking the input from the data set file. Then the data set is allocated to different resources in the methods. Each method performs a different algorithm. The result for each type of algorithm is compared and then the result will be represented graphically.
 ![image](https://user-images.githubusercontent.com/112755503/214497309-be752552-544e-455c-88c7-5fc1dd92de36.png)

Fig 1: Overall Flow of the Project
 
![image](https://user-images.githubusercontent.com/112755503/214497363-8a081d19-d84c-47ce-950e-0bf164ef1791.png)

   Fig 2:  Detailed overall flow of the project
...
MULTILEVEL QUEUE SCHEDULING - This algorithm introduced a modified Max-min algorithm [8]. This algorithm selects the task with the average or nearest to average execution time instead of selecting the largest task for execution. This selected task is then scheduled over the resource with minimum completion time. This algorithm reduces the overall makespan and also balances load across resources.
